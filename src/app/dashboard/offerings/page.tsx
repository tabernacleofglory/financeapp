...
export default function OfferingsPage() {
  ...
  React.useEffect(() => {
    ...
    const fetchData = async () => {
        ...
        const currentPeriodTotal = currentPeriodEntries.filter(e => e.category === 'Offerings').reduce((sum, entry) => sum + (entry.amount || 0), 0);
        const previousPeriodTotal = previousPeriodEntries.filter(e => e.category === 'Offerings').reduce((sum, entry) => sum + (entry.amount || 0), 0);
        ...
        const aggregatedCampusData = campuses.map(campus => {
            ...
            const thisWeekReported = thisWeekCampusEntries.filter(e => e.category === 'Offerings').reduce((sum, entry) => sum + (entry.amount || 0), 0);
            const thisWeekAttendance = thisWeekCampusEntries.filter(e => e.category === 'Attendance').reduce((sum, entry) => sum + (entry.amount || 0), 0);
            
            const lastWeekReported = lastWeekCampusEntries.filter(e => e.category === 'Offerings').reduce((sum, entry) => sum + (entry.amount || 0), 0);
    ...
    <>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
            <div className="lg:col-span-1">
                <Card>
                    <CardHeader>
                        <CardTitle className="tracking-wider">OFFERINGS</CardTitle>
                    </CardHeader>
